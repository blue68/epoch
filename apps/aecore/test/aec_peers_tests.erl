...
aec_peers_test_() ->
    CustomPoolSetup = fun() ->
        setup([{peer_pool, [{standby_times, [0, 1]}, {max_rejections, 3}], []}])
    end,
    [{setup, fun global_setup/0, fun global_teardown/1, [
        ...
        {setup, fun setup/0, fun teardown/1, fun test_invalid_hostname/0},
        ...
        {setup, CustomPoolSetup, fun teardown/1, fun test_connection_failure/0},
        {setup, CustomPoolSetup, fun teardown/1,
                {timeout, 8, fun test_connection_down/0}},
        {setup, fun() -> setup([{sync_max_inbound, 10, 100}]) end, fun teardown/1,
            fun test_inbound_connections/0},
        {setup, fun() -> setup([{sync_max_outbound, 3, 10}]) end, fun teardown/1,
            fun test_outbound_connections/0},
        {setup, fun() -> setup([{ping_interval, 500, 120000}]) end, fun teardown/1,
            fun test_ping/0},
        {setup, fun setup/0, fun teardown/1, fun test_connection_conflict/0},
        {setup, fun() -> setup([{peer_unblock_interval, 4000, 900000}]) end,
                fun teardown/1, fun test_blocking/0}
    ]}
].
...
test_multiple_normal_peers() ->
    ...
    {ok, Conn1} = ?assertCalled(connect, [#{ r_pubkey := PubKey1 }], {ok, _}, 3100),
    {ok, Conn2} = ?assertCalled(connect, [#{ r_pubkey := PubKey2 }], {ok, _}, 3100),
    {ok, Conn3} = ?assertCalled(connect, [#{ r_pubkey := PubKey3 }], {ok, _}, 3100),
...
test_invalid_hostname() ->
    ...
    PubKey = <<"ef42d46eace742cd">>,
    Id = aec_peers:peer_id(PubKey),
    Peer = peer(PubKey, "15e6a562-98ea-435d-ae6a-6b3b9a6d3f32", 4000),
...
test_address_group_selection() ->
    ...
    {ok, Conn1} = ?assertCalled(connect, [#{ r_pubkey := PubKey1 }], {ok, _}, 100),
    ok = conn_peer_connected(Conn1),
...
test_connection_closed() ->
    ...
    {ok, Conn1} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 100),
    ok = conn_peer_connected(Conn1),
    ...
    % Check it is reconnects after only one second
...
test_connection_failure() ->
    ...
    % Check it is retried after 5 seconds
    {ok, Conn2} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    ok = conn_peer_connected(Conn2),
...
test_connection_down() ->
    ...
    % Check second retry.
    {ok, Conn3} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    ok = conn_peer_connected(Conn3),
    conn_kill(Conn3),
    ...
    % Check last retry.
    {ok, Conn4} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    ok = conn_peer_connected(Conn4),
    conn_kill(Conn4),

    % Peer is downgraded to the unverified pool.
    ...
    {ok, Conn5} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    conn_kill(Conn5),

    {ok, Conn6} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    conn_kill(Conn6),

    {ok, Conn7} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    conn_kill(Conn7),
    ...
    {ok, Conn8} = ?assertCalled(connect, [#{ r_pubkey := PubKey }], {ok, _}, 1100),
    conn_kill(Conn8),
...
test_inbound_connections() ->
    ...
    {ok, Conn11} = test_mgr_start_inbound(Peer11),
    ?assertEqual(temporary, conn_peer_accepted(Conn11)),
    ?assertEqual(11, aec_peers:count(connections)),
    conn_connection_closed(Conn11),
...
proc_test_mgr_loop(S) ->
    receive
        ...
        {From, stop, []} ->
            #{ procs := Procs } = S,
            lists:foreach(fun conn_stop/1, Procs),
            reply(From, ok)
    end.
