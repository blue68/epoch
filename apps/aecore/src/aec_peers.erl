...
-record(state, {
    ...
    last_connect_time :: undefined | non_neg_integer(),
    %% The blocked peers.
...
%% @docs Encodes peer info map to a peer URI.
...
handle_call({block_peer, PeerInfo}, _From, State0) ->
    ...
    {reply, ok, update_peer_metrics(maybe_connect(State))};
...
handle_call({peer_connected, PeerId, PeerCon}, _From, State0) ->
    ...
    {reply, Result, update_peer_metrics(maybe_connect(State))};
...
handle_cast({add_peer, SourceAddr, Peer}, State0) ->
    ...
    {noreply, update_peer_metrics(maybe_connect(State))};
...
-spec count(connections | inbound | outbound | peers
            | verified | unverified | standby, state())
    -> non_neg_integer().
...
%% Schedules a peer connection if not already sheduled.
-spec maybe_connect(state()) -> state().
maybe_connect(#state{ connect_ref = undefined } = State) ->
...
next_connect_delay(#state{ last_connect_time = undefined }) -> 0;
next_connect_delay(State) ->
    case is_outbound_allowed(State) of
        ...
        true ->
            ...
            max(0, BoundDelay - (timestamp() - LastTime))
    end.
...
start_connection(Peer, State) ->
    ...
    {ok, Pid} = aec_peer_connection:connect(ConnOpts),
    Conn = #conn{ peer = Peer, type = outbound, state = connecting, pid = Pid },
    ...
    case conn_find(PeerId, State) of
        ...
        {ok, #conn{ state = ConnState, type = Type, pid = OldPid } = OldConn}
          when Pid =/= OldPid ->
            ...
            State2 = conn_del(PeerId, pool_release(PeerId, State)),
...
max_inbound() ->
    aeu_env:user_config_or_env([<<"sync">>, <<"max_inbound">>],
                               aecore, sync_max_inbound, ?DEFAULT_MAX_INBOUND).

max_outbound() ->
    aeu_env:user_config_or_env([<<"sync">>, <<"max_outbound">>],
                               aecore, sync_max_outbound, ?DEFAULT_MAX_OUTBOUND).
