...
register_oracle(_Cfg) ->
    ...
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY, [SignedTx], ?MINER_PUBKEY, [], Trees, Height, ?PROTOCOL_VERSION),
...
extend_oracle(Cfg) ->
    ...
    SignedTx = aetx_sign:sign(Tx, PrivKey),
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY, [SignedTx], ?MINER_PUBKEY, [], Trees, CurrHeight, ?PROTOCOL_VERSION),
...
query_oracle(Cfg, Opts) ->
    ...
    {ok, [SignedTx], Trees2} =
        aec_trees:apply_signed_txs(key, ?MINER_PUBKEY, [SignedTx], ?MINER_PUBKEY, [], Trees, CurrHeight, ?PROTOCOL_VERSION),
