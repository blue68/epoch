...
preclaim(Cfg) ->
    ...
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY_PRECLAIM, [SignedTx], ?MINER_PUBKEY_PRECLAIM, [], Trees, Height, ?PROTOCOL_VERSION),
...
claim(Cfg) ->
    ...
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY_CLAIM, [SignedTx], ?MINER_PUBKEY_CLAIM, [], Trees, Height, ?PROTOCOL_VERSION),
    S2 = aens_test_utils:set_trees(Trees1, S1),
...
update(Cfg) ->
    ...
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY_UPDATE, [SignedTx], ?MINER_PUBKEY_UPDATE, [], Trees, Height, ?PROTOCOL_VERSION),
...
transfer(Cfg) ->
    ...
    {ok, [SignedTx], Trees2} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY_TRANSFER, [SignedTx], ?MINER_PUBKEY_TRANSFER, [], Trees1, Height, ?PROTOCOL_VERSION),
...
revoke(Cfg) ->
    ...
    {ok, [SignedTx], Trees1} = aec_trees:apply_signed_txs(key, ?MINER_PUBKEY_REVOKE, [SignedTx], ?MINER_PUBKEY_REVOKE, [], Trees, Height, ?PROTOCOL_VERSION),
