...
handle_info({noise, _, <<Type:16, Payload/binary>>}, S) ->
    case aec_peer_messages:deserialize(Type, Payload) of
        ...
        {stop, _Vsn, _Msg} ->
            #{ status := {connected, ESock} } = S,
            aec_peers:connection_closed(peer_id(S), self()),
            epoch_sync:debug("Connection closed by ~p", [maps:get(host, S)]),
            enoise:close(ESock),
            {stop, normal, S};
        {MsgType, Vsn, Msg} ->
...
maybe_close(#{ keep_alive := false, status := {connected, _} } = S) ->
    notify_close(S);
maybe_close(S) ->
    S.
    ...
    case is_non_registered_accept(S) of
        true ->
            ...
            case aec_peers:peer_accepted(PeerInfo, self()) of
                permanent ->
                    {ok, NewS#{ keep_alive := true }};
                temporary ->
                    {ok, NewS#{ keep_alive := false }};
                {error, _} = Error ->
